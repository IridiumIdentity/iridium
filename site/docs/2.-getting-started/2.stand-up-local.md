...
Clone the repo
```shell
$ git clone git@github.com:IridiumIdentity/iridium.git && cd iridium
```
...
`cd` into the Angular project within the Iridium project
```shell
...
a social provider (such as GitHub) to access the system.  

From here, you can create single page applications or server apps in the UI to be secured by Iridium.
