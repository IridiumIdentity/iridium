...
the database.  If you used the Docker Compose to stand up the Iridium the password in this case will be `iridium` .
```shell
$ Enter value for --password (the database password): 
```

Next, it will ask for your admin user password.  You may choose any value you like.
This is the password you will use along with the `--admin-email` argument you passed to the Iridium CLI to
init the system.  You will use these credentials to sign into the Iridium management UI.
```shell
