...
public class AuthenticationApiClient {
    ...
    public AuthenticationApiClient(final String authenticationBaseUrl, final RestTemplate restTemplate) {
        super();
        this.restTemplate = restTemplate;
        this.authenticationBaseUrl = authenticationBaseUrl;
    }

    public ApiDataResponse<IdentityResponse> requestWithShortLivedToken(final String token, final String subdomain) {
        final var restGetter = new RestGetter<ApiDataResponse<IdentityResponse>>(new ErrorHandler(), restTemplate);
        ...
        return restGetter.get(url, headers, new ParameterizedTypeReference<ApiDataResponse<IdentityResponse>>() {});
    }
    ...
    public ApiDataResponse<AuthenticationResponse> authenticate(final AuthenticationRequest request) {
        final var restPoster = new RestPoster<AuthenticationRequest, ApiDataResponse<AuthenticationResponse>>(restTemplate);
        final var headers = new HttpHeaders();
        headers.set("Accept", AuthenticationRequest.MEDIA_TYPE);
        headers.set("Content-Type", AuthenticationResponse.MEDIA_TYPE);
        final var url = String.format(IDENTITIES_PATH_FMT, authenticationBaseUrl);
        return restPoster.post(url, headers, request, new ParameterizedTypeReference<ApiDataResponse<AuthenticationResponse>>() {}, new ErrorHandler());
    }
}
