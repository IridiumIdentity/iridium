...
class AuthorizationServiceTest {
    ...
    @Mock
    private ProviderUrlGenerator mockUrlGenerator;
    @Mock
    private ProviderAccessTokenRequestor mockAccessTokenRequestor;
    @Mock
    private IdentityEntityRepository mockIdentityRepository;
    @Mock
    private IdentityResponseMapper mockIdentityResponseMapper;
    @Mock
    private IdentityEntityInstantiator mockIdentityInstantiator;
    @Mock
    private IdentityEntity mockIdentity;
    @Mock
    private HttpServletRequest mockServletRequest;
    @Mock
    private AuthorizationGrantTypeParamValidator mockGrantTypeValidator;
    @Mock
    private ApplicationEntityRepository mockApplicationRepository;
    @Mock
    private AuthorizationRequestParameterValidator mockRequestParameterValidator;
    @Mock
    private AuthorizationCodeEntityInstantiator mockAuthCodeInstantiator;
    @Mock
    private AuthorizationCodeEntityRepository mockAuthCodeRepository;
    @Mock
    private SuccessAuthorizationParameterGenerator mockSuccessParamGenerator;
    @Mock
    private RedirectUrlGenerator mockRedirectUrlGenerator;
    @Mock
    private TenantEntityRepository mockTenantRepository;
    @Mock
    private ProviderProfileRequestor mockProviderProfileRequestor;
    @Mock
    private IdentityEmailEntityRepository mockEmailRepository;
    @Mock
    private SubdomainExtractor mockSubdomainExtractor;
    @Mock
    private AuthenticationEntityRepository mockAuthenticationRepository;
    @InjectMocks
    ...
    public void authorizeWithProvider_UserDoesExistAllGood_BehavesAsExpected() {
        ...
        when(mockAccessTokenRequestor.requestAccessToken(same(providerUrl))).thenReturn(authorizationResponse);
        when(mockProviderProfileRequestor.requestGithubProfile(same(providerProfileUrl), same(providerAccessToken))).thenReturn(profileResponse);
        when(mockEmailRepository.findByEmailAddressAndIdentity_ParentTenantId(same(githubResponseEmail), same(tenantId))).thenReturn(Optional.of(email));
        ...
        verify(mockProviderProfileRequestor).requestGithubProfile(same(providerProfileUrl), same(providerAccessToken));
        verify(mockEmailRepository).findByEmailAddressAndIdentity_ParentTenantId(same(githubResponseEmail), same(tenantId));
    ...
    public void authorizeWithProvider_UserDoesNotExistCreatesNewAllGood_BehavesAsExpected() {
        ...
        when(mockAccessTokenRequestor.requestAccessToken(same(providerUrl))).thenReturn(authorizationResponse);
        when(mockProviderProfileRequestor.requestGithubProfile(same(providerProfileUrl), same(providerAccessToken))).thenReturn(profileResponse);
        when(mockEmailRepository.findByEmailAddressAndIdentity_ParentTenantId(same(githubResponseEmail), same(tenantId))).thenReturn(Optional.empty());
        when(mockIdentityInstantiator.instantiateFromGithub(same(profileResponse), same(externalProvider))).thenReturn(mockIdentity);
    ...
    public void completeAuthorizationWithProvider_ApplicationNotFound_ExceptionThrown() {
        ...
        final var exception = assertThrows(ResourceNotFoundException.class, () -> subject.completeAuthorizationWithProvider(code, providerName, clientId, state));
    ...
    public void completeAuthorizationWithProvider_TenantNotFound_ExceptionThrown() {
        ...
        assertThat(exception.getMessage(),  is(equalTo(String.format("tenant %s not found with clientId: %s", tenantId, clientId))));
    ...
    public void completeAuthorizationWithProvider_CodeIsBlank_ExceptionThrown() {
        ...
        final var exception = assertThrows(IllegalArgumentException.class, () -> subject.completeAuthorizationWithProvider(code, providerName, clientId, state));
    ...
    public void authorize_AllGood_BehavesAsExpected() {
        ...
        when(mockAuthenticationRepository.findFirstByAuthTokenAndExpirationAfter(same(userToken), any(Date.class))).thenReturn(Optional.of(authentication));
        when(mockRequestParameterValidator.validateAndOptionallyRedirect(same(redirectUri), same(params))).thenReturn("");
        when(mockIdentity.getAuthorizedApplications()).thenReturn(authorizedApplications);
        when(mockAuthCodeInstantiator.instantiate(same(mockIdentity), same(params))).thenReturn(authorizationCodeEntity);
        when(mockAuthCodeRepository.save(same(authorizationCodeEntity))).thenReturn(authorizationCodeEntity);
        when(mockSuccessParamGenerator.generate(same(params), same(authorizationCode))).thenReturn(paramMap);
        when(mockRedirectUrlGenerator.generate(same(redirectUri), same(paramMap))).thenReturn(generatedRedirectUri);
        ...
        verify(mockAuthenticationRepository).findFirstByAuthTokenAndExpirationAfter(same(userToken), any(Date.class));
        verify(mockApplicationRepository).findByClientId(same(clientId));
        verify(mockRequestParameterValidator).validateAndOptionallyRedirect(same(redirectUri), same(params));
    ...
    public void authorize_RedirectsBecauseOfError_BehavesAsExpected() {
        ...
        when(mockAttributeValidator.isNotBlank(anyString())).thenCallRealMethod();
        when(mockAttributeValidator.equals(anyString(), anyString())).thenCallRealMethod();
        when(mockRequestParameterValidator.validateAndOptionallyRedirect(same(redirectUri), same(params))).thenReturn(errorRedirectUri);
    ...
    public void authorize_ApplicationNotFound_ExceptionThrown() {
        ...
        final var exception = assertThrows(ResourceNotFoundException.class, () -> subject.authorize(formRequest, params, mockServletRequest));
        ...
        verify(mockAuthenticationRepository).findFirstByAuthTokenAndExpirationAfter(same(userToken), any(Date.class));
        verify(mockApplicationRepository).findByClientId(same(clientId));
        assertThat(exception.getMessage(), is(equalTo("application not found for client_id: " + clientId)));
    }
}
