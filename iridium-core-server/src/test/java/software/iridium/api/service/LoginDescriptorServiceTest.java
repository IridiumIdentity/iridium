...
class LoginDescriptorServiceTest {
    ...
    @Mock
    private LoginDescriptorEntityRepository mockLoginDescriptorRepository;
    @Mock
    private LoginDescriptorResponseMapper mockResponseMapper;
    @Mock
    private AttributeValidator mockAttributeValidator;
    @InjectMocks
    private LoginDescriptorService subject;
    ...
    public void getBySubdomain_AllGood_BehavesAsExpected() {
        ...
        when(mockLoginDescriptorRepository.findByTenant_Subdomain(same(subdomain))).thenReturn(Optional.of(loginDescriptor));
    ...
    public void getBySubdomain_LoginDescriptorNotFound_ExceptionThrown() {
        ...
        when(mockAttributeValidator.isValidSubdomain(same(subdomain))).thenReturn(true);
        when(mockLoginDescriptorRepository.findByTenant_Subdomain(same(subdomain))).thenReturn(Optional.empty());

        final var exception = assertThrows(ResourceNotFoundException.class, () -> subject.getBySubdomain(subdomain));

        verify(mockAttributeValidator).isValidSubdomain(same(subdomain));
        verify(mockLoginDescriptorRepository).findByTenant_Subdomain(same(subdomain));
        assertThat(exception.getMessage(), is(equalTo("login descriptor not found for subdomain: " + subdomain)));
    }
}
