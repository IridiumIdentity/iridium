...
class AuthenticationServiceTest {
    ...
    @Mock
    private AuthenticationRequestValidator mockRequestValidator;
    @Mock
    private ApplicationEntityRepository mockApplicationRepository;
    @Mock
    private AuthenticationRequestParamValidator mockAuthRequestParamValidator;
    @Mock
    ...
    public void authenticate_AllGood_BehavesAsExpected() {
        ...
        when(mockEmailRepository.findByEmailAddressAndIdentity_ParentTenantId(same(username), same(tenantId))).thenReturn(emailOpt);
        ...
        when(mockApplicationRepository.findByClientId(same(applicationClientId))).thenReturn(Optional.of(application));
        ...
        verify(mockEmailRepository).findByEmailAddressAndIdentity_ParentTenantId(same(username), same(tenantId));
    ...
    public void authenticate_IdentityIsLocked_ExceptionThrown() {
        ...
        final var exception = assertThrows(NotAuthorizedException.class, () -> subject.authenticate(request, params));
    ...
    public void authenticate_emailNotFoundWithApplicationId_ExceptionThrown() {
        ...
        when(mockEmailRepository.findByEmailAddressAndIdentity_ParentTenantId(same(username), same(tenantId))).thenReturn(Optional.empty());
        when(mockApplicationRepository.findByClientId(same(applicationClientId))).thenReturn(Optional.of(application));
    ...
    public void authenticate_ApplicationNotFoundWithClientId_ExceptionThrown() {
        ...
        when(mockApplicationRepository.findByClientId(same(applicationClientId))).thenReturn(Optional.empty());

        final var exception = assertThrows(ResourceNotFoundException.class, () -> subject.authenticate(request, params));

        verify(mockRequestValidator).validate(request);
        verify(mockAuthRequestParamValidator).validate(same(params));
        verify(mockApplicationRepository).findByClientId(same(applicationClientId));
        assertThat(exception.getMessage(), is(equalTo("application not found for client id: " + applicationClientId)));
    }
}
