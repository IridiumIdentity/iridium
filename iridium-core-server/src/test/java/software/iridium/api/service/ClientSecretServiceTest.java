...
class ClientSecretServiceTest {
    ...
    @Mock
    private ClientSecretEntityInstantiator mockClientSecretInstantiator;
    @Mock
    private ClientSecretCreateResponseMapper mockResponseMapper;
    @Mock
    private ClientSecretEntityRepository mockClientSecretRepository;
    @Mock
    private EncoderUtils mockEncoderUtils;
    @InjectMocks
    private ClientSecretService subject;
    ...
    public void create_AllGood_BehavesAsExpected() throws NoSuchAlgorithmException {
        ...
        when(mockApplicationRepository.findById(same(applicationId))).thenReturn(Optional.of(application));
        when(mockClientSecretInstantiator.instantiate(same(application), anyString())).thenReturn(clientSecret);
        when(mockResponseMapper.map(same(clientSecret), anyString())).thenReturn(response);
        when(mockEncoderUtils.cryptoSecureToHex(same(ClientSecretService.SEED_LENGTH))).thenCallRealMethod();
    ...
    public void create_ApplicationNotFoundForId_ExceptionThrown() {
        ...
        final var exception = assertThrows(ResourceNotFoundException.class, () -> subject.create(applicationId));

        verify(mockAttributeValidator).isUuid(same(applicationId));
        verify(mockApplicationRepository).findById(same(applicationId));

        assertThat(exception.getMessage(), is(equalTo("application not found for id: " + applicationId)));
    }
    ...
    public void create_ApplicationDoesNotRequireSecret_ExceptionThrown() {
        ...
        final var exception = assertThrows(ClientCallException.class, () -> subject.create(applicationId));

        verify(mockAttributeValidator).isUuid(same(applicationId));
        verify(mockApplicationRepository).findById(same(applicationId));

        assertThat(exception.getMessage(), is(equalTo("incorrect attempt to create secret for non secret application: " + applicationId)));
    }
    ...
    public void create_EncoderError_ExceptionThrown() throws NoSuchAlgorithmException {
        ...
        when(mockAttributeValidator.isUuid(same(applicationId))).thenReturn(true);
        when(mockApplicationRepository.findById(same(applicationId))).thenReturn(Optional.of(application));
        when(mockEncoderUtils.cryptoSecureToHex(same(ClientSecretService.SEED_LENGTH))).thenThrow(NoSuchAlgorithmException.class);
    ...
    public void delete_AllGood_BehavesAsExpected() {
        ...
        when(mockClientSecretRepository.findById(same(clientSecretId))).thenReturn(Optional.of(clientSecret));
    ...
    public void delete_SecretNotAssociatedWithApplication_ExceptionThrown() {
        ...
        final var exception = assertThrows(IllegalArgumentException.class, () -> subject.delete(applicationId, clientSecretId));

        verify(mockAttributeValidator).isUuid(same(applicationId));
        verify(mockAttributeValidator).isUuid(same(clientSecretId));
        verify(mockClientSecretRepository).findById(same(clientSecretId));
        assertThat(exception.getMessage(), is(equalTo(String.format("client secret %s not associated with application %s", clientSecretId, applicationId))));
    }
}
