...
class AuthorizationControllerTest {
    ...
    @InjectMocks
    private AuthorizationController subject;
    ...
    public void proxyAuthorizationRequestToProvider_AllGood_BehavesAsExpected() {
        ...
        final var response = subject.proxyAuthorizationRequestToProvider(
                mockModelMap,
                mockAttributes,
                responseType,
                state,
        ...
        verify(mockAuthorizationService).proxyAuthorizationRequestToProvider(
                same(responseType),
                same(state),
                same(redirectUri),
                same(clientId),
    ...
    public void authorize_AllGood_BehavesAsExpected() {
        ...
        when(mockAuthorizationService.authorize(same(formRequest), same(params), same(mockServletRequest))).thenReturn(redirectUri);

        final var response = subject.authorize(mockServletRequest, formRequest, params);

        verify(mockAuthorizationService).authorize(same(formRequest), same(params), same(mockServletRequest));
