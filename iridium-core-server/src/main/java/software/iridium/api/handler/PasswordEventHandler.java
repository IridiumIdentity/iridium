...
public class PasswordEventHandler {
    ...
    @Resource
    private EmailSendRequestInstantiator emailInstantiator;
    @Resource
    private SelfUrlGenerator urlGenerator;
    @Resource
    private EmailApiClient emailApiClient;

    @Transactional(propagation = Propagation.REQUIRED)
    public void handlePasswordResetInitiatedEvent(final IdentityEntity identity, final String clientId) {
        // todo (josh fischer) implement
        Map<String, Object> props = new HashMap<>();
        props.put("forgotPasswordLink", urlGenerator.generateChangePasswordUrl(identity.getPasswordResetToken(), clientId));

        //final var sendRequest = emailInstantiator.instantiate(identity, "Password Reset Notification", props);
