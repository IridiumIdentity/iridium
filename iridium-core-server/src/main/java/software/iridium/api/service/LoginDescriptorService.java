...
public class LoginDescriptorService {
    ...
    @Resource
    private LoginDescriptorEntityRepository loginDescriptorRepository;
    @Resource
    private LoginDescriptorResponseMapper responseMapper;
    @Resource
    ...
    public LoginDescriptorResponse getBySubdomain(final String subdomain) {
        ...
        final var loginDescriptor = loginDescriptorRepository.findByTenant_Subdomain(subdomain)
                .orElseThrow(() -> new ResourceNotFoundException("login descriptor not found for subdomain: " + subdomain));

        return responseMapper.map(loginDescriptor);
    }

    public LoginDescriptorResponse create(final LoginDescriptorCreateRequest request, final String tenantId) {
        return null;
    }
}
