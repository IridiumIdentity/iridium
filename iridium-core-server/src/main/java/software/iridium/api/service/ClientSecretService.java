...
public class ClientSecretService {
    ...
    @Resource
    private ClientSecretEntityInstantiator clientSecretInstantiator;
    @Resource
    private ClientSecretCreateResponseMapper responseMapper;
    @Resource
    private ClientSecretEntityRepository clientSecretRepository;
    @Resource
    ...
    public ClientSecretCreateResponse create(final String applicationId) {
        ...
        if (application.getApplicationType().doesNotRequireSecret()) {
            logger.error("incorrect attempt to create secret for non secret application type {}", application.getId());
            throw new ClientCallException("incorrect attempt to create secret for non secret application: " + application.getId());
        }
    ...
    public void delete(final String applicationId, final String clientSecretId) {
        ...
        final var clientSecret = clientSecretRepository.findById(clientSecretId)
                .orElseThrow(() -> new ResourceNotFoundException("client secret not found for id: " + clientSecretId));

        if (clientSecret.getApplication().getId().equals(applicationId)) {
            clientSecretRepository.delete(clientSecret);
            return;
        }
        throw new IllegalArgumentException(String.format("client secret %s not associated with application %s", clientSecretId, applicationId));
    }
}
