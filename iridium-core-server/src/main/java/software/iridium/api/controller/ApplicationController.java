...
public class ApplicationController {
    ...
    @PostMapping(value = "/tenants/{tenant-id}/applications", consumes = ApplicationCreateRequest.MEDIA_TYPE, produces = ApplicationCreateResponse.MEDIA_TYPE)
    public ApiDataResponse<ApplicationCreateResponse> create(@RequestBody final ApplicationCreateRequest request, @PathVariable("tenant-id") final String tenantId) {
        return new ApiDataResponse<>(applicationService.create(request, tenantId));
    }

    @GetMapping(value = "/tenants/{tenant-id}/applications/{application-type-id}", produces = ApplicationSummary.MEDIA_TYPE_LIST)
    public PagedListResponse<ApplicationSummary> getPageByTenantAndApplicationType(
        ...
        return applicationService.getPageByTenantIdAndApplicationTypeId(tenantId, applicationTypeId, page, size, active);
    }
}
