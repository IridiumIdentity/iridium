...
public class TemplateController {
    ...
    @GetMapping("/login")
    public String retrieveLoginForm(final AuthenticationRequest request,
                                    final Model model,
                                    final @RequestParam Map<String, String> params,
                                    final HttpServletRequest servletRequest) {
        logger.info("loading login for: {}", servletRequest.getRequestURL().toString());
        logger.info("loading login for subdomain: {}", servletRequest.getRequestURL().toString().split("\\.")[0]);
    ...
    @GetMapping("/register")
    public String register(final CreateIdentityRequest createIdentityRequest, final Model model, final HttpServletRequest servletRequest) {
        logger.info("loading login for: {}", servletRequest.getRequestURL().toString());
        logger.info("loading login for subdomain: {}", servletRequest.getRequestURL().toString().split("\\.")[0]);
    ...
    public String retrieveAuthorizationPage(ModelMap model, HttpServletRequest servletRequest, HttpServletResponse servletResponse) {
